...
# =============================================================================
# 1. SYSTEM CONFIGURATION (Geometric RS-3 Omega)
# =============================================================================
CONFIG = {
    "grid_size": 12,               # Small grid for fast resonance
    "num_agents": 4,               # Parallel agents
    "latent_dim": 128,             # High capacity brain
    "bottleneck_dim": 32,          
    "total_steps": 30000,          # Reduced for demo speed (was 60k)
    "learning_rate": 1e-4,         
    "contrastive_weight": 3.0,     # Enforce internal consistency
    "spectral_weight": 0.8,        # Keep weight matrix complex (SVD)
    "vision_gate_weight": 60.0,    
    "alignment_weight": 40.0,      
    "proprioceptive_weight": 35.0, 
    "field_coupling": 0.40,        # Autonomous Search Regime
    "oracle_steps": 3000,          # Supervised warm-up steps
    "base_entropy": 0.3,           
    "bias_decay_end": 10000,       
    "starvation_limit": 500,       
    "model_path": "rs3_geometric_brain.pth",
...
class PerformanceMonitor:
    ...
    def log_status(self, step, succs, morts, starve_counts, envs, agents, telemetry):
        ...
        print(f" VISION SNR   | {snr:7.2f} [{self.sparkline(list(self.snr_history))}] | Oracle: {'ON' if step < CONFIG['oracle_steps'] else 'OFF'}")
        print("-" * 100)
        print(f" ID  | LEN | CERT | GAZE (X,   Y)   | SPECTRAL | STATUS")
        print("-" * 100)
        ...
        for i in range(CONFIG["num_agents"]):
            ...
            print(f" A{i}  | {len(e.snake):03d} | {t['ct']:.2f} | [{gx:+.2f}, {gy:+.2f}] |  {a.spectral_entropy:.2f}    | {status}")
...
class AgentNetwork(nn.Module):
    def __init__(self, config):
        ...
        # The "Paths" (Symmetric Projections)
    ...
    def compute_centroids(self, logits, head_pos):
        ...
        # Softmax Temperature 150.0 forces "Sharp" attention (Coordinates)
    ...
    def forward(self, x, head_pos):
        x_proc = x.clone()
        # Non-Duality Filter: Subtract Self from World to see pure Target
        x_proc[:, 2, :, :] -= x_proc[:, 0, :, :] 
...
class SnakeEnv:
    ...
    def get_obs(self):
        ...
        obs[1, :, :] = 0.1
        obs[2, self.food[0], self.food[1]] = 10.0 # High contrast food
        return obs
        
    def render_ascii(self):
        grid = np.full((self.size, self.size), "·")
        grid[self.food[0], self.food[1]] = "★"
        for i, (sx, sy) in enumerate(self.snake): grid[sx, sy] = "H" if i==0 else "O"
...
def train():
    ...
    if os.path.exists(CONFIG["model_path"]):
        ...
        try:
            ...
            for a in agents: a.load_state_dict(loaded_state, strict=False)
        except: pass
    ...
    print("--- RS-3 OMEGA PROTOCOL STARTED ---")

    for step in range(CONFIG["total_steps"]):
        # Bias decays over time to allow autonomous behavior
        ...
        for i, agent in enumerate(agents):
            ...
            # Heuristic Compass (The "Instinct")
            ...
            # 1. Oracle Loss (Coordinate Grounding)
            ...
            # 2. Contrastive Loss (Internal vs External Consistency)
            l_unity = CONFIG["contrastive_weight"] * (F.mse_loss(z_i, z_e) + F.mse_loss(z_i, z_p))
            
            # 3. Spectral Loss (Prevent Mode Collapse via SVD)
            ...
            step_logs.append({
                ...
                'val': pred_val, 'gaze': [tx, ty]
            })

        # Global Field Update (Hive Mind Effect)
...
def play_demo():
    ...
    for _ in range(200):
        ...
        # Telemetry
        print(f"Gaze Vector: [{target_v[0,0]:.2f}, {target_v[0,1]:.2f}]")
        print(f"Proprioception: [{ego_v[0,0]:.2f}, {ego_v[0,1]:.2f}]")
        time.sleep(0.1)
        if done: break
